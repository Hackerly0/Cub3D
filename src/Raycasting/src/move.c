...
/*   Updated: 2025/10/12 17:59:34 by salshaha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_player(t_cub *cub, float angle)
{
	...
	cos_angle = cos(angle);
	sin_angle = sin(angle);
	cub->game->dir_x = cub->game->dir_x * cos_angle - cub->game->dir_y
		* sin_angle;
	cub->game->dir_y = old_dir_x * sin_angle + cub->game->dir_y * cos_angle;
	cub->game->plane_x = cub->game->plane_x * cos_angle - cub->game->plane_y
		* sin_angle;
	cub->game->plane_y = old_plane_x * sin_angle + cub->game->plane_y
		* cos_angle;
}
...
void	handle_movement_keys(t_cub *cub)
{
	if (mlx_is_key_down(cub->game->mlx, MLX_KEY_W))
		move_player(cub, cub->game->dir_x * MOVE_SPEED, cub->game->dir_y
			* MOVE_SPEED);
	if (mlx_is_key_down(cub->game->mlx, MLX_KEY_S))
		move_player(cub, -cub->game->dir_x * MOVE_SPEED, -cub->game->dir_y
			* MOVE_SPEED);
	if (mlx_is_key_down(cub->game->mlx, MLX_KEY_A))
		move_player(cub, cub->game->dir_y * MOVE_SPEED, -cub->game->dir_x
			* MOVE_SPEED);
...
void	keyhook(void *param)
{
	...
	{
		cub->textures->player->instances[0].x = (int)(cub->game->xp_pos
